    ...
    methods
        ...
        function updateDirectGeometry(self, q)
            ...
            for i = 1:self.jointNumber
                if self.jointType(i) == 0 % Rotation joint
                    rot = [cos(q(i)), -sin(q(i)), 0; sin(q(i)), cos(q(i)), 0; 0, 0, 1];
                    self.iTj(:,:,i) = self.iTj_0(:,:,i) * [rot, [0; 0; 0]; 0, 0, 0, 1];
                elseif self.jointType(i) == 1 % Prismatic joint
                    self.iTj(:,:,i) = self.iTj_0(:,:,i) * [eye(3), [0; 0; q(i)]; 0, 0, 0, 1];
                end
            end
        end
        ...
        function [kTj] = getTransformWrtRandomFrame(self, k, j)
            bTk = self.getTransformWrtBase(k);
            bTj = self.getTransformWrtBase(j);
